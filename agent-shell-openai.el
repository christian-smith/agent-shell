...
(declare-function agent-shell-codex-app-server-make-client "agent-shell-codex-app-server")
(declare-function agent-shell-codex-app-server-handle-busy-prompt "agent-shell-codex-app-server")
(declare-function agent-shell-make-agent-config "agent-shell")
(autoload 'agent-shell-make-agent-config "agent-shell")
(autoload 'agent-shell-codex-app-server-make-client "agent-shell-codex-app-server")
...
Returns an agent configuration alist using `agent-shell-make-agent-config'."
  ...
  (agent-shell-make-agent-config
   ...
   :busy-prompt-handler (when (eq agent-shell-openai-codex-transport 'app-server)
                          #'agent-shell-codex-app-server-handle-busy-prompt)
   :client-maker (lambda (buffer)
                   (agent-shell-openai-make-codex-client :buffer buffer))
   :install-instructions (if (eq agent-shell-openai-codex-transport 'app-server)
                             "Install Codex CLI and ensure `codex app-server` is available."
                           "See https://github.com/agentclientprotocol/codex-acp for installation.")))

(defun agent-shell-openai-start-codex ()
  "Start an interactive Codex agent shell."
  (interactive)
  (let ((mark-active (if (and (eq agent-shell-openai-codex-transport 'app-server)
                              (region-active-p)
                              (equal (point) (mark t)))
                         nil
                       mark-active)))
...
Uses `agent-shell-openai-authentication' for authentication configuration."
  ...
  (cond
   ((eq agent-shell-openai-codex-transport 'app-server)
