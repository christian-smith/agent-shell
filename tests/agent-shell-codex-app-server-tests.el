...
(ert-deftest agent-shell-codex-app-server-reconnect-preserves-buffer ()
  "Reconnect swaps clients only after the existing thread resumes."
    ...
    (with-temp-buffer
      ...
      (let* ((old (agent-shell-codex-app-server-make-client :command "sh"))
        ...
        (cl-letf (((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                  ...
                  ((symbol-function 'agent-shell-codex-app-server--send-rpc-request)
                   (lambda (&rest args) (setq resume args)))
          ...
          (should (eq (map-elt agent-shell--state :client) old))
          ...
          (when (eq outcome 'busy)
            (map-put! old :active-turn-id "new-turn"))
          ...
          (should (= (length stopped) 1))
          (should (eq (eq (map-elt agent-shell--state :client) old)
                      (not (eq outcome 'success))))
