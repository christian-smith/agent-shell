...
(ert-deftest agent-shell-codex-app-server-async-questions-do-not-block ()
  ...
  (let ((client (agent-shell-codex-app-server-make-client :command "sh"))
        ...
        notified)
    (unwind-protect
        (let ((agent-shell-codex-app-server-question-hook
               (list (lambda (context) (push context notified)))))
          ...
          (should (eq (map-elt (car notified) :client) client))
          (should (eq (map-elt (car notified) :buffer) (car displayed)))
          (should (equal (map-elt (car notified) :questions) (map-elt item 'questions)))
...
(ert-deftest agent-shell-codex-app-server-async-questions-have-independent-status ()
  "Dismissing one question must not disable another question's Answer button."
  (let ((client (agent-shell-codex-app-server-make-client :command "sh"))
        (agent-shell-codex-app-server-question-hook nil)
        displayed)
    (unwind-protect
        (cl-letf (((symbol-function 'display-buffer)
                   (lambda (buffer &rest _) (push buffer displayed))))
          (dolist (id '("first" "second"))
            (agent-shell-codex-app-server--show-async-questions
             client `((id . ,id) (questions . (((title . "Which color?")))))))
          (let ((first (gethash "first" (map-elt client :async-questions)))
                (second (gethash "second" (map-elt client :async-questions))))
            (with-current-buffer (map-elt first :buffer)
              (goto-char (point-min))
              (search-forward "Dismiss")
              (button-activate (button-at (1- (point)))))
            (should (eq (map-elt first :status) 'dismissed))
            (should (eq (map-elt second :status) 'pending))
            (cl-letf (((symbol-function 'completing-read)
                       (lambda (&rest _) (signal 'quit nil))))
              (with-current-buffer (map-elt second :buffer)
                (goto-char (point-min))
                ...
                (condition-case nil
                  ...
                  (quit nil))))
            (should (eq (map-elt second :status) 'pending))))
      (dolist (buffer displayed)
        (when (buffer-live-p buffer) (kill-buffer buffer))))))

(ert-deftest agent-shell-codex-app-server-reconnect-preserves-buffer ()
  "Reconnect releases the old writer before resuming and preserves the buffer."
  (dolist (outcome '(failure success busy))
    (with-temp-buffer
      (insert "Conversation\nDraft")
        ...
        (cl-letf (((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                  ...
                  ((symbol-function 'agent-shell-codex-app-server--send-rpc-request)
                   (lambda (&rest args)
                     ...
                     (setq resume args)))
                  ((symbol-function 'agent-shell-codex-app-server-shutdown)
          ...
          (when (eq outcome 'busy)
            (map-put! (map-elt agent-shell--state :client) :active-turn-id "new-turn"))
          (if (memq outcome '(success busy))
              (funcall (plist-get resume :on-success) '((thread . ((id . "thread-1")))))
            (funcall (plist-get resume :on-failure) '((message . "Login required"))))
          (should (equal (buffer-string) "Conversation\nDraft"))
          (should (= (length stopped) (if (eq outcome 'success) 1 2)))
          (should (equal (map-elt (map-elt agent-shell--state :client) :thread-id) "thread-1"))
          (should-not (map-elt (map-elt agent-shell--state :client) :reconnecting))
          (should (eq (eq (car stopped) old) (eq outcome 'success)))
          (should-not (map-elt old :reconnecting)))))))

(ert-deftest agent-shell-codex-app-server-show-account-test ()
  "Account inspection uses the running client and never refreshes credentials."
  (with-temp-buffer
    (let* ((client (agent-shell-codex-app-server-make-client :command "sh"))
           (agent-shell--state (list (cons :client client)))
           request notice)
      (cl-letf (((symbol-function 'agent-shell-codex-app-server--client-started-p)
        ...
        (agent-shell-codex-app-server-show-account)
        (should (eq (plist-get request :client) client))
        (should (equal (plist-get request :method) "account/read"))
        (should (equal (plist-get request :params)
                       `((refreshToken . ,(agent-shell-codex-app-server--json-bool nil)))))
        (dolist (entry '((((type . "chatgpt") (email . "user@example.com") (planType . "pro"))
                         "user@example.com (pro)")
                        (((type . "apiKey")) "apiKey")
                        (nil "Not signed in")))
          (funcall (plist-get request :on-success) (list (cons 'account (car entry))))
          (should (string-suffix-p (cadr entry) notice)))
        (funcall (plist-get request :on-failure) nil)
        (should (string-suffix-p "Could not read the running Codex account" notice))))))

(ert-deftest agent-shell-codex-app-server-show-account-rejects-other-buffers ()
  "Account inspection cannot start a process from an unrelated buffer."
  (with-temp-buffer
    (let ((agent-shell--state nil))
      (should-error (agent-shell-codex-app-server-show-account) :type 'user-error))))

(ert-deftest agent-shell-codex-app-server-reconnect-rejects-active-turn ()
  "Reconnect cannot interrupt an active turn."
  (with-temp-buffer
    (let* ((client (agent-shell-codex-app-server-make-client :command "sh"))
           (agent-shell--state (list (cons :client client))))
      (map-put! client :thread-id "thread-1")
      (map-put! client :active-turn-id "turn-1")
      (should-error (agent-shell-codex-app-server-reconnect) :type 'user-error))))
