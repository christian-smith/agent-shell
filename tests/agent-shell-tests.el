...
(ert-deftest agent-shell--set-model-id-config-option-test ()
  ...
  (let* ((initial-config-options [((id . "model")
    ...
    (cl-letf (((symbol-function 'agent-shell--state)
              ...
              ((symbol-function 'agent-shell--update-header-and-mode-line)
               #'ignore))
      (agent-shell--set-model-id :model-id "gpt-5.5")
...
(ert-deftest agent-shell--set-mode-id-config-option-test ()
  ...
  (let* ((initial-config-options [((id . "mode")
    ...
    (cl-letf (((symbol-function 'agent-shell--state)
              ...
              ((symbol-function 'agent-shell--update-header-and-mode-line)
               #'ignore))
      (agent-shell--set-mode-id :mode-id "auto")
      (should (equal (map-elt sent-request :method)
                     "session/set_config_option"))
      (should (equal (map-nested-elt sent-request '(:params configId))
                     "mode"))
...
(ert-deftest agent-shell--set-model-id-legacy-fallback-test ()
