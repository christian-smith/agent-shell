...
After switching stored credentials externally (for example, with =codex-auth switch=), run =M-x agent-shell-codex-app-server-reconnect= in an idle Codex shell. It starts a replacement app-server and resumes the same thread without replacing the buffer or replaying history. Active requests and unanswered questions must finish first. If initialization or resume fails or takes longer than 60 seconds, the original connection is retained. Background terminals and browser connections may be lost after a successful reconnect. This command does not switch accounts itself or change other running shells; environment-based authentication and the configured credential store still determine which credentials Codex uses.
