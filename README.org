...
Alternatively, install the [[https://developers.openai.com/codex/cli/][Codex CLI]] and use its native app-server transport. Set =(setq agent-shell-openai-codex-transport 'app-server)= before starting Codex. The default remains =acp=.

Codex can ask asynchronous questions while continuing its work. These appear in a separate question buffer without taking focus. Select =Answer= to choose suggested answers or enter your own text. Answers use the prompt queue: they steer a running turn or start a new turn when Codex is idle. Dismissing a question does not interrupt Codex.

To disable command approvals and sandboxing for new or resumed Codex app-server sessions, set =(setq agent-shell-openai-codex-app-server-yolo-mode t)=. This allows commands and file changes outside the workspace without asking. The default is nil; running sessions and other agents are unchanged. MCP forms and user-input questions can still require a response.

After switching stored credentials externally (for example, with =codex-auth switch=), run =M-x agent-shell-codex-app-server-reconnect= in an idle Codex shell. It initializes a replacement app-server, stops the old process to release the thread's writer lock, and resumes the same thread without replacing the buffer or replaying history. Active requests and unanswered questions must finish first. Initialization failure retains the original connection. Resume failure retains the buffer but leaves it disconnected; run reconnect again to retry. The operation times out after 60 seconds. Background terminals and browser connections may be lost when the old process stops. This command does not switch accounts itself or change other running shells; environment-based authentication and the configured credential store still determine which credentials Codex uses.
...
Per-session always-approve or auto permission mode can also be set with =agent-shell-xai-yolo-mode= and =agent-shell-xai-auto-mode= (sent as =session/new= =_meta=). Session modes are =default=, =plan=, and =ask=. Set =agent-shell-text-file-capabilities= to nil to make Grok use its own filesystem instead of loading files into Emacs buffers.

Reasoning effort uses Grok's native =reasoning_effort= config option and requires a build that advertises it. Mid-turn messages use the =_x.ai/interject= ACP extension; Grok queues them for the next safe point in its running turn.
...
| agent-shell-openai-codex-app-server-command   | Command and parameters for the OpenAI Codex app-server client.                  |
| agent-shell-openai-codex-environment          | Environment variables for the OpenAI Codex client.                              |
| agent-shell-openai-codex-transport            | Transport used for the OpenAI Codex integration.                                |
...
| agent-shell-tool-use-expand-by-default        | Boolean or predicate controlling which tool use sections expand by default.     |
| agent-shell-tool-use-minimum-visible-seconds  | Minimum seconds an automatically expanded tool remains visible.                |
...
| agent-shell-ui-mode-hook                      | Hook run after entering or leaving ‘agent-shell-ui-mode’.                       |
 ...
 | agent-shell-xai-auto-mode                     | When non-nil, start Grok sessions in auto permission mode.                      |
...
| agent-shell-xai-yolo-mode                     | When non-nil, start Grok sessions in always-approve mode.                       |
...
| C-c /           | agent-shell-prompt-queue                                | Queue or immediately send a prompt depending on shell busy state.             |
